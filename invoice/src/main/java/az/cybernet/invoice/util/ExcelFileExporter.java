    ...
    public static byte[] exportInvoicesToExcel(List<FilterResponse> invoices, String[] headers) {
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            for (FilterResponse invoice : invoices) {
                ...
                setCell(row, 2, invoice.getSenderId() == null ? "" : invoice.getSenderId());
                setCell(row, 3, invoice.getCustomerId() == null ? null : invoice.getCustomerId());
                setCell(row, 4, invoice.getTotal());
